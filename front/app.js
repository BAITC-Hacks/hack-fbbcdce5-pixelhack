(() => {
  ...
  function renderProposal(action) {
    if (!action) return; const section = document.createElement("section"); section.className = "proposal";
  ...
  async function submit(event) {
    ...
    try { let response; if (text) response = await request(`/sessions/${session.session_id}/chat`, { method: "POST", headers: authHeaders({ "Content-Type": "application/json" }), body: JSON.stringify({ message: text }) }); if (file) { addMessage(`Файл: ${file.name}`, "user"); response = await sendAttachment(file); elements.file.value = ""; elements.preview.hidden = true; } if (response) renderResponse(response); setStatus("Подключено", "online"); }
    catch (error) { addMessage(error.message); setStatus("Нужно обновить сессию", "demo"); } finally { elements.send.disabled = false; }
  }
  async function init() { try { await ensureSession(); const health = await request("/health"); const cart = await request(`/sessions/${session.session_id}/cart`, { headers: authHeaders() }); setCart(cart); setStatus(health.assistant_mode === "demo" ? "Демо-режим" : "Подключено", health.assistant_mode === "demo" ? "demo" : "online"); } catch (error) { clearSession(); setStatus("Сервис недоступен"); addMessage(error.message); } }
