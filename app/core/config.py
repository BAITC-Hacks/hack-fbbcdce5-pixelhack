...
class Settings(BaseSettings):
    ...
    ekt_search_pages: int = Field(default=400, ge=1, le=2000)
    request_timeout_seconds: float = Field(default=15, gt=0, le=120)
    chat_timeout_seconds: float = Field(default=120, gt=0, le=180)
    session_ttl_seconds: int = Field(default=3600, ge=60)
    max_sessions: int = Field(default=1000, ge=1, le=10000)

    @model_validator(mode="after")
    def validate_credentials(self):
        if not (self.ekt_username and self.ekt_password.get_secret_value()):
